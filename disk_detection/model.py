...
def _multi_scale_deform_attn_pytorch(value, spatial_shapes, sampling_locations, attention_weights):
    """Pure PyTorch fallback for MSDeformAttnFunction.

    It is slower than the compiled CUDA/C++ op, but keeps inference usable when the
    packaged .pyd was built against a different PyTorch/CUDA ABI.
    """
    batch_size, _, n_heads, head_dim = value.shape
    ...
    output = value.new_zeros(batch_size, n_queries, n_heads, head_dim)

    for level, (height, width) in enumerate(spatial_shapes.tolist()):
        value_l = value_list[level].flatten(2).transpose(1, 2)
        value_l = value_l.reshape(batch_size * n_heads, head_dim, height, width)
        grid_l = sampling_grids[:, :, :, level].permute(0, 2, 1, 3, 4)
        grid_l = grid_l.reshape(batch_size * n_heads, n_queries, n_points, 2)
        ...
        sampled_l = sampled_l.reshape(batch_size, n_heads, head_dim, n_queries, n_points)
        sampled_l = sampled_l.permute(0, 3, 1, 4, 2)
        weights_l = attention_weights[:, :, :, level].unsqueeze(-1)
        output = output + (sampled_l * weights_l).sum(dim=3)

    return output.reshape(batch_size, n_queries, n_heads * head_dim)
...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index):
        ...
        if MSDeformAttnFunction is None:
            output = _multi_scale_deform_attn_pytorch(
